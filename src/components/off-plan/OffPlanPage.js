...
const heroImage = "https://ik.imagekit.io/kc8kqzi2u/Grove%20Vista%20Properties/Listing/off-plan.webp";
const PRICE_RANGES = ["Any Price", "Under 1 Cr", "1 Cr – 3 Cr", "3 Cr – 7 Cr", "Above 7 Cr"];
...
function withinPriceRange(priceCr, range) {
  ...
  if (range === "Under 1 Cr") return priceCr < 1;
  if (range === "1 Cr – 3 Cr") return priceCr >= 1 && priceCr <= 3;
  if (range === "3 Cr – 7 Cr") return priceCr > 3 && priceCr <= 7;
  if (range === "Above 7 Cr") return priceCr > 7;
  return true;
}
...
export default function OffPlanPage() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setPage(1);
      try {
        const res = await fetch(`/api/properties?category=off-plan&propertyFor=${propertyFor}&page=1&limit=${limit}`);
  ...
  useEffect(() => {
    const slug = `off-plan-${propertyFor}`;
  ...
  const handleLoadMore = async () => {
    ...
    try {
      const next = page + 1;
      const res = await fetch(`/api/properties?category=off-plan&propertyFor=${propertyFor}&page=${next}&limit=${limit}`);
      const data = await res.json();
          ...
          <div className="grid items-center gap-12 lg:grid-cols-2">
            ...
            <div className="relative mt-12 lg:mt-0">
              ...
              <div className="absolute -bottom-5 -right-2 sm:-right-5 rounded-2xl border border-white/10 bg-slate-800 px-6 py-4 shadow-2xl">
                ...
                <p className="text-[11px] font-medium text-slate-400">Active Projects</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-screen-2xl px-6 py-3 sm:px-10 lg:px-8 2xl:px-10">
          <div className="flex items-center gap-2">
            <div className="flex flex-1 min-w-0 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 focus-within:border-amber-400 focus-within:bg-white transition-all">
              ...
              <input
                type="text"
                placeholder="Search by city, developer or project…"
      ...
      <main className="mx-auto max-w-screen-2xl px-6 py-10 sm:px-10 lg:px-8 2xl:px-10">
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-slate-200 py-28 text-center">
            <p className="text-lg font-bold text-slate-800 mb-1">No projects match your search</p>
